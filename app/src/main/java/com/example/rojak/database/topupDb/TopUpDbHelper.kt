    ...
    companion object {
        ...
        private const val SQL_CREATE_ENTRIES =
            "CREATE TABLE ${TopUpContract.TopUpEntry.TABLE_NAME} (" +
                    ...
                    "${TopUpContract.TopUpEntry.COLUMN_NAME_CURRENT_WALLET_AMOUNT} NUMERIC);"

        private const val SQL_DELETE_ENTRIES = "DROP TABLE IF EXISTS" +
                " ${TopUpContract.TopUpEntry.TABLE_NAME}"

        private const val SQL_INITIALIZE_WALLET = "INSERT INTO ${TopUpContract.TopUpEntry.TABLE_NAME} (" +
                "${TopUpContract.TopUpEntry.COLUMN_NAME_TOPUP_AMOUNT}," +
                "${TopUpContract.TopUpEntry.COLUMN_NAME_CURRENT_WALLET_AMOUNT}) VALUES" +
                "(0.0, 0.0);"
    }

    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL(SQL_CREATE_ENTRIES)
        db.execSQL(SQL_INITIALIZE_WALLET)
    }
